...
const AllUsers = () => {
   ...
   const handleMakeAdmin = user => {
      console.log(user);
      fetch(`http://localhost:5333/users/admin/${user._id}`, {
         method: "PATCH",
      })
         .then(res => res.json())
         .then(data => {
                        ...
                        <td>
                           {
                              user.role === 'admin' ? <button className="btn btn-xs">admin</button> : <button
                                 ...
                                 className="btn btn-ghost bg-black btn-md btn-xs"><FaUsers className="text-lg text-white" /></button>
                           }
                        </td>
                        <td>
                           <button onClick={() => handleDelete(user)} className="btn btn-ghost bg-red-500 btn-md btn-xs">
